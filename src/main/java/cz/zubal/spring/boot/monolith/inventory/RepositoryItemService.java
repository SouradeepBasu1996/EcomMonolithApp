...
public class RepositoryItemService implements ItemService {
  ...
  @Override
  public boolean isCustomerAllowedToBuy(String username, String itemUuid) {
    Customer customer = customerService.findByUsername(username);
    Item item = findByUuid(itemUuid);

    // Example rule: premium items only for registered customers
    return !item.isPremium() || customer.isPremiumMember();
  }

  @Override
  public double applyCustomerDiscount(String username, Item item) {
    Customer customer = customerService.findByUsername(username);

    if (customer.isPremiumMember()) {
      return item.getPrice() * 0.90;  // 10% discount
    }
